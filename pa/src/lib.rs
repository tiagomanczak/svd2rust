...
mod imp {
	pub use super::{Width, ResetValue};

	impl<U, REG> super::Width for Reg<U, REG> where U: Copy {
		type Type = U;
	}
	impl<U, REG> super::Readable for Reg<U, REG> where Self: Readable {
		type Reader = R<U, REG>;
	}
	impl<U, REG> super::Writable for Reg<U, REG> where Self: Writable {
		type Writer = W<U, REG>;
	}

	impl <U, REG> super::ReadRegister for Reg<U, REG> where Self: Readable {
		#[inline(always)]
		fn read(&self) -> Self::Reader {
			self.read()
		}
